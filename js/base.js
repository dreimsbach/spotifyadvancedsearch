...
async function handleAppleMusicClick(e) {
  ...
  button.innerHTML = '<svg class="w-5 h-5 inline-block animate-spin" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" fill="none"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg> Searching...';
  ...
  try {
    ...
    if (data.results && data.results.length > 0) {
      window.open(data.results[0].collectionViewUrl, '_blank');
    } else {
      button.innerHTML = originalText;
      alert('Album not found on Apple Music');
    }
  } catch (error) {
    console.error('Error searching Apple Music:', error);
    button.innerHTML = originalText;
    alert('Error searching Apple Music. Please try again.');
  }
